...
struct SuppliedParameters: Codable {
    var hostName: String?
    var exptName: String?
    var portNumber: Int?
    var ncoFreq: Int?
    ...
    var tauD: Int?
    var tauDelay: Int?
    var progSatArray: [Int]?          // V suffix used later indicates a parameter whose value is changed (to a local variable)
    var userTag: String?
    
    mutating func defaults() -> Void {
        if hostName == nil { hostName = redPitayaIp }
        if exptName == nil { exptName = "FID" }
        if portNumber == nil { portNumber = 1001 }
        if ncoFreq == nil { ncoFreq = 16004000 }            // para1
        if stepFreq == nil { stepFreq = 0 }                 // para4
        if pulseLength == nil { pulseLength = 5000 }        // para6
        if pulseStep == nil { pulseStep = 0 }               // para7
        if noScans == nil { noScans = 1 }                   // para8
        if noExpts == nil { noExpts = 1 }                   // para7
        if rptTime == nil { rptTime = 1000  }               // para8
        if tauTime == nil { tauTime = 0 }                   // para9
        if tauInc == nil { tauInc = 0 }                     // para10
        if noData == nil { noData = 5000 }                  // para11
        if delayInSeconds == nil { delayInSeconds = 1 }     // para12
        if tauD == nil { tauD = 0 }                         // para13
        if tauDelay == nil { tauDelay = 0 }                 // para14
        if progSatArray == nil { progSatArray = [1] }
        if userTag == nil { userTag = "" }
        
                // V suffix used later indicates a parameter whose value is changed (to a local variable)
    }
    
    enum CodingKeys: String, CodingKey {
        case hostName = "hostname"
        case exptName = "exptname"
        case portNumber = "portnumber"
        case ncoFreq = "ncofreq"
        case stepFreq = "stepfreq"
        case pulseLength = "pulselength"
        case pulseStep = "pulsestep"
        case noScans = "noscans"
        case noExpts = "noexpts"
        ...
        case delayInSeconds = "delayinseconds"
        case tauD = "taud"
        case tauDelay = "taudelay"
        case progSatArray = "progsatarray"
        case userTag = "usertag"
    }
}

struct NMRResult: Codable {
    //var parameters : WorkingParameters?
    var parameters : SuppliedParameters?
    var datapoints : [[Int16]]
}

@objc class NMRServer: NSObject {
...
//  Line  D03 Hardware Pin 5 for field gradients
    ...
    func experiment(_ parameters: SuppliedParameters) -> [[Int16]]
    {
        self.nmrResult.parameters = parameters
        self.nmrResult.datapoints.removeAll(keepingCapacity: true)
        /*
        do {
            let encoder = JSONEncoder()
            encoder.outputFormatting = .prettyPrinted
            try
            self.parametersString = String(data: encoder.encode(self.nmrResult.parameters!), encoding: .utf8)
        } catch {
            
        }
         */
        self.nmrResult.parameters?.defaults()
        if rpServer() {
            return nmrResult.datapoints
        } else {
            return []
        }
    }
    
    @objc func experiment(_ definition: NSString) -> [[Int16]] {
        let defData = String(definition).data(using: .utf8)!
        do {
            let def = try JSONDecoder().decode([SuppliedParameters].self, from: defData)
            return experiment(def[0])
        }
        catch {
            return []
        }
    ...
    @objc func expFID() -> [[Int16]] {
    
        if nmrResult.parameters == nil {
            nmrResult.parameters = SuppliedParameters()
        }
        nmrResult.parameters!.hostName = redPitayaIp
        nmrResult.parameters!.portNumber = 1001
        nmrResult.parameters!.ncoFreq = 12400405 //16004000 //12360100
        nmrResult.parameters!.stepFreq  = 0
        nmrResult.parameters!.pulseLength = 5000
        nmrResult.parameters!.pulseStep  = 0
        nmrResult.parameters!.noScans = 1 // Changed from 4
        nmrResult.parameters!.noExpts = 1
        nmrResult.parameters!.rptTime = 1000
        nmrResult.parameters!.tauTime = 0
        nmrResult.parameters!.tauInc = 0
        nmrResult.parameters!.tauDelay = 0
        nmrResult.parameters!.exptName = "FID"
        nmrResult.parameters!.progSatArray = [1]
        nmrResult.parameters!.noData = 5000
        nmrResult.parameters!.delayInSeconds = 1
        nmrResult.parameters!.tauD=0
        nmrResult.parameters!.userTag = ""
    ...
    func rpServer() -> Bool {
        ...
        var tauTimeV = nmrResult.parameters!.tauTime     // parameters that are changed
        var rptTimeV = nmrResult.parameters!.rptTime
        var tauDV = nmrResult.parameters!.tauD
        var retval = true
        var trnrFailed = false
        
        nmrResult.datapoints.removeAll(keepingCapacity: true)
        retResult = true
        retError = ""
        ...
        func updateBuf2(_ scanCounter: Int) -> Void {
            ...
            func experimentFID() -> Void {
                ...
                hwCtrl[ 8] = PA_OFF | BL_ON  | ADC_START; duration[ 8] = 125 * nmrResult.parameters!.noData!
                                                                                  - DC; reVal[ 8] = RF_OFF
            ...
            func experimentCPMGT2() -> Void {
                ...
                /* Start of CPMG sequence*/
                
                hwCtrl[253]  = PA_ON  | BL_OFF | ADC_STOP;  duration[253] = 125 * 10 - DC;              reVal[253]=RF_OFF;   /* wait 10  PA to come on*/
                hwCtrl[254]  = PA_ON  | BL_OFF | ADC_STOP;  duration[254] = pl - DC;                    reVal[254]=8100;     /* TX=8100 90 pulse us */
                hwCtrl[255]  = PA_OFF | BL_OFF | ADC_STOP;  duration[255] = 125 * (tau - 10) - pl - DC; reVal[255]=RF_OFF;   /* tau delay 1 ms corr 10 us son*/
                ...
                //for (ii = 0; ii < BLKS/2; ii+=8)
                    let noData2 = 50
                    hwCtrlC[ii]   = PA_ON  | BL_OFF | ADC_STOP;  durationC[ii]   = 125 * 10 - DC;                           reValC[ii]=RF_OFF;     /* wait 10  PA to come on*/
                    hwCtrlC[ii+1] = PA_ON  | BL_OFF | ADC_STOP;  durationC[ii+1] = pl * 2 - DC;                             reValC[ii+1]=0; imValC[ii+1]=8100; /* 180 90Phase*/
                     
                    hwCtrlC[ii+2] = PA_OFF | BL_OFF | ADC_STOP;  durationC[ii+2] = 125 * (tau - noData2 / 2) - pl * 2 - DC; reValC[ii+2]=RF_OFF;     /* delay before Echo*/
                    hwCtrlC[ii+3] = PA_OFF | BL_ON  | ADC_START; durationC[ii+3] = 125 * noData2 - DC;                      reValC[ii+3]=RF_OFF;   /*  collect data for no_Data2 */
                    hwCtrlC[ii+4] = PA_OFF | BL_OFF | ADC_STOP;  durationC[ii+4] = 125 * (tau - 10 - noData2 / 2 - 3) - DC; reValC[ii+4]=RF_OFF;   /*  delay after Echo*/
                    hwCtrlC[ii+5] = PA_OFF | BL_OFF | ADC_STOP;  durationC[ii+5] = 125 * 1 - DC;                            reValC[ii+5]=RF_OFF;   /*  Fill*/
                    hwCtrlC[ii+6] = PA_OFF | BL_OFF | ADC_STOP;  durationC[ii+6] = 125 * 1 - DC;                            reValC[ii+6]=RF_OFF;   /*  Fill*/
                    hwCtrlC[ii+7] = PA_OFF | BL_OFF | ADC_STOP;  durationC[ii+7] = 125 * 1 - DC;                            reValC[ii+7]=RF_OFF;   /*  Fill*/
                ...
                for ii  in 0...(BLKS / 2 - 1) {
                //for (ii = 0; ii < BLKS/2; ii++)
                    bufCPMG[2 * ii + 1] = UInt32((hwCtrlC[ii] & 0xff) << 24) | UInt32((durationC[ii] & 0xffffff));
                    bufCPMG[2 * ii + 2] = UInt32(reValC[ii] & 0xffff) | UInt32(((imValC[ii] & 0xffff) << 16)) ;
                }
            }
            ...
            func experimentT1() -> Void {
                ...
                for ii in 2...203 {
                    hwCtrl[ii] = PA_OFF | BL_OFF | ADC_STOP; duration[ii] = 125 * tauDV! - DC; reVal[ii]=RF_OFF;
                }
                    /*END LOOP*/
                ...
                hwCtrl[206] = PA_OFF | BL_ON  | ADC_START; duration[206]  = 125 * nmrResult.parameters!.noData! - DC;
            ...
            func experimentSE() -> Void {
                ...
                hwCtrl[11] = PA_OFF | BL_ON  | ADC_START; duration[11] = 125 * nmrResult.parameters!.noData! - DC;
            ...
            pl = nmrResult.parameters!.pulseLength! / 8
            if pl > 3000 { pl = 3000 } // 3000*8/1000 = 24 microseconds
            if pl == 0 { pl = 750 } // 6 microseconds 750/125
        
            var d = Double(nmrResult.parameters!.ncoFreq!)
            d = d / 125.0e6
            d = d * 256.0 * 256.0 * 256.0 * 256.0
            d = d + 0.5
            bufTrnr[0] = UInt32(floor(d))
            // bufTrnr[0] = 549824533
            //bufTrnr[0] = UInt32(floor(Double(parameters.ncoFreq)/125.0e6*256.0*256.0*256.0*256.0+0.5))
            ...
            switch nmrResult.parameters!.exptName {
            case "FID":
                experimentFID()
            case "CPMG":
                experimentCPMG()
        ...
        func trnr() -> Bool {
            ...
            func canSend() -> Void {
                ...
                if !trnrFailed {
                    ...
                    case 0:
                        ...
                        iii = 0
                    case 1:
                        if iii < (nmrResult.parameters!.noScans! * nmrResult.parameters!.noExpts!) {
                            if iii == 0 {
            ...
            func didFail() -> Void {
                xmtSocket!.stop()
                trnrFailed = sendStep < 3
                signalXmt()
                retResult = false
                retError = xmtSocket!.retError
            }
        
            if nmrResult.parameters!.exptName == "PROG_SAT" {
                trnrFailed = true
                retResult = false
                retError = "No code to handle PROG_SAT"
                return false
            } // cannot find any values for prog_sat_delay
            
            xmtSocket = TcpSocket(hostName: nmrResult.parameters!.hostName!,
                                  hostPort: nmrResult.parameters!.portNumber!,
        ...
        func rcvr() -> Bool {
            ...
            func didReceive(data: Data, isComplete: Bool) {
                ...
                if trnrFailed {
                    ...
                    if nmrResult.datapoints.count >= expectedNodeCount ||
                       ((endTime.uptimeNanoseconds - startTime.uptimeNanoseconds) / UInt64(1e9)) > nmrResult.parameters!.delayInSeconds! {
                        rcvSocket!.stop()
                        signalRcv()
                    } else {
                        if isComplete {
            ...
            let expectedNodeCount = (nmrResult.parameters!.noData! + (BUFSIZE - 1)) / BUFSIZE
            ...
            func buildIntegers() -> Void {
                ...
                while arrayIndex < rcvba!.count {
                    ...
                    case 1:
                        ...
                        rcvIx += 1
                        if rcvIx >= BUFSIZE {
                            nmrResult.datapoints.append(rcvData)
...
//            Thread.sleep(until: deadline)
            ...
            rcvSocket = TcpSocket(hostName: nmrResult.parameters!.hostName!,
                                  hostPort: nmrResult.parameters!.portNumber!,
                                  canSend: canSend, didReceive: didReceive, didFail: didFail)
...
//        if nmrResult.parameters!.exptName == "CPMG" { noEchoes = nmrResult.parameters!.tauInc }
        if nmrResult.parameters!.exptName == "CPMGX" { noEchoes = nmrResult.parameters!.tauInc! }
        
        if tauDV! > 100000 { tauDV! = 100000 }
        rptTimeV! *= 1000 // change to micro seconds?
        t1Guess = tauTimeV!
        if t1Guess > 2000 { t1Guess = 2000 }
        if tauTimeV! < 25 && tauTimeV! > 0 { tauTimeV! = 25 }
        pl = nmrResult.parameters!.pulseLength!
        tau = tauTimeV!
        ...
        //signalRcv()   // required for possible swift bug
        //signalXmt()   // required for possible swift bug
    ...
    func resultAsJSON() -> String {
        //let result = ResultAsJSON(parameters: parameters, datapoints: retData)
        let encoder = JSONEncoder()
        encoder.outputFormatting = .sortedKeys
        do {
            let data = try encoder.encode(nmrResult)
